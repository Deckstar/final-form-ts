...
/**
 *
 * A component that subscribes to form state, and injects
 * both form state and the `form` instance via a render
 * prop.
 *
 * The `<FormSpy/>` will rerender any time the form state
 * it is subscribed to changes. By default it subscribes
 * to _all_ form state. You can control which form state
 * it subscribes to with the `subscription` prop.
 *
 * By providing an `onChange` prop, `<FormSpy/>` can also
 * be used to execute code when a particular part of form
 * state changes.
 *
 * ## Props
 *
 * `<FormSpy/>` accepts `FormSpyProps` and will call the
 * render function with `FormSpyRenderProps`.
 *
 * The only required prop is one of `onChange`,
 * `component`, `render`, or `children`.
 *
 * ## Basic Usage
 *
 * It should be noted that `<FormSpy/>` is for very
 * advanced use cases.
 *
 * **If you are not restricting your form state by
 * providing a `subscription` prop to `<Form/>`, you
 * probably do not need `<FormSpy/>`!** Just use the form
 ...
 *
 * There are three ways to render a `<FormSpy/>`
 * component:
 *
 * | Prop                   | Type                  |
 * | ---------------------- | --------------------- |
 ...
 * `component` prop, it will be rendered with [`React.createElement()`](https://reactjs.org/docs/react-api.html#createelement),
 ...
 *
 * `<FormSpy/>` can sometimes be useful to execute code
 * when a particular part of form state changes. This is
 * what the `onChange` callback is for.
 *
