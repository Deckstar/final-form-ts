 ...
 *
 * Unsubscribed to items will be potentially `undefined`, unless they are not
 * part of the `SubscribableType` (i.e. should always be present on the state).
 *
 *     ---
 *
 * @param State The state that can be subscribed to.
 * @param Subscription An object of booleans. `true` values will be subscribed
 *   to, while `false` or missing values will be `undefined`.
 * @param [SubscribableType] (Optional) — Indicates which keys can be
 *   subscribed to. Keys in `State` that are not found in `SubscribableType`
 *   will remain typed as they are on the `State` object. By default, the whole
 *   `State` is treated as subscribable. Thus, if `SubscribableType` is omitted,
 *   all keys in `StateBasedOnSubscription` could potentially be `undefined` if
 *   they are not marked in `Subscription` as `true`.
 */
...
/**
 * By default: all values are subscribed. If subscription is specified, some
...
> {
  ...
  /**
   * `true` if the form values are different from the values it was initialized
  ...
  dirty: boolean;
  /**
   * An object full of booleans, with a value of `true` for each `dirty` field.
   * _Pristine fields will not appear in this object_.
   *
   * Note that this is a flat object, so if your field name is
   * `addresses.shipping.street`, the `dirty` value for that field will be
   * available under `dirty['addresses.shipping.street']`.
   */
  dirtyFields: FormDirtyState<FormValues>;
  /**
   * An object full of booleans, with a value of `true` for each field that has
   * a different value from the one when the form was last submitted. _Pristine
   * (since last submit) fields will not appear in this object_.
   *
   * Note that this is a flat object, so if your field name is
   * `addresses.shipping.street`, the `dirtySinceLastSubmit` value for that
   * field will be available under `dirty['addresses.shipping.street']`.
   */
  dirtyFieldsSinceLastSubmit: FormDirtyState<FormValues>;
  /**
   * `true` if the form values are different from the values it was last
   * submitted with. `false` otherwise.
   *
   * Comparison is done with shallow-equals.
   */
  dirtySinceLastSubmit: boolean;
  /**
   * The whole-form error returned by a validation function under the
   * `FORM_ERROR` key.
   */
  error: any;
  /**
   * An object containing all the current validation errors. The shape will
   * match the shape of the form's values.
   */
  errors: ValidationErrors;
  /**
   * `true` when the form currently has submit errors. Useful for distinguishing
   * _why_ `invalid` is `true`.
   */
  hasSubmitErrors: boolean;
  /**
   * `true` when the form currently has validation errors. Useful for
   * distinguishing _why_ `invalid` is `true`.
   *
   * For example, if your form is `invalid` because of a submit error, you might
   * also want to disable the submit button if user's changes to fix the submit
   * errors causes the form to have sync validation errors.
   */
  ...
  /**
   * `true` if any of the fields or the form has a validation or submission
   * error. `false` otherwise.
   *
   * Note that a form can be invalid even if the errors do not belong to any
   * currently registered fields.
   */
  invalid: boolean;
  /**
   * An object full of booleans, with a boolean value for each field name
   * denoting whether that field is `modified` or not.
   *
   * Note that this is a flat object, so if your field name is
   * `addresses.shipping. street`, the `modified` value for that field will be
   * available under `modified['addresses.shipping.street']`.
   */
  modified: FormBooleanState<FormValues>;
  /**
   * `true` if the form values have ever been changed since the last submission.
   * `false` otherwise.
   */
  modifiedSinceLastSubmit: boolean;
  /**
   * `true` if the form values are the same as the initial values. `false`
   * otherwise.
   *
   * Comparison is done with shallow-equals.
   */
  pristine: boolean;
  /**
   * A top-level status object that you can use to represent form state that
   * can't otherwise be expressed/stored with other methods.
   *
   * Some examples where this can be useful:
   *
   * - For tracking "step stages" in a multi-step wizard form;
   * - For capturing and passing through API responses to your inner component;
   */
  ...
  /**
   * An object containing all the current submission errors. The shape will
   * match the shape of the form's values.
   */
  ...
  /**
   * An object full of booleans, with a boolean value for each field name
   * denoting whether that field is `touched` or not.
   *
   * Note that this is a flat object, so if your field name is
   * `addresses.shipping.street`, the `touched` value for that field will be
   * available under `touched['addresses.shipping.street']`.
   */
  touched: FormBooleanState<FormValues>;
  /**
   * `true` if neither the form nor any of its fields has a validation or
  ...
  /**
   * An object full of booleans, with a boolean value for each field name
   * denoting whether that field is `visited` or not.
   *
   * Note that this is a flat object, so if your field name is
   * `addresses.shipping.street`, the `visited` value for that field will be
   * available under `visited['addresses.shipping.street']`.
   */
...
/**
 * `FieldState` is an object containing the following values. **Depending on
 * your subscription when calling
 * {@linkcode FormApi.registerField | form.registerField()}, some of the values
 * may not be present.**
 */
export interface FieldState<FieldValue = any> {
  ...
  /**
   * `true` when the value of the field is not equal to the initial value (using
   * the {@linkcode FieldConfig.isEqual | isEqual} comparator provided at field
   * registration), `false` if the values are equal.
   */
  dirty: boolean;
  /**
   * `true` when the value of the field is not equal to the value last submitted
   * (using the {@linkcode FieldConfig.isEqual | isEqual} comparator provided at
   * field registration), `false` if the values are equal.
   */
  dirtySinceLastSubmit: boolean;
   ...
   *
   * Once `true`, it will remain `true` for the lifetime of the field, or until
   * the form or field state is reset.
   */
  modified: boolean;
  /**
   * `true` if this field's value has ever been changed since the last
   * submission. `false` otherwise.
   *
   * Once `true`, it will remain `true` until the next submit action, or until
   * the form or field state is reset.
   */
  modifiedSinceLastSubmit: boolean;
  ...
  name: string;
  /**
   * `true` if the current value is `===` to the initial value, `false` if the
   * values are `!==`.
   */
  pristine: boolean;
  ...
  touched: boolean;
  /**
   * `true` if this field has no validation or submission errors. `false`
   * otherwise.
   */
  valid: boolean;
  /**
   * `true` if this field is currently waiting on its asynchronous field-level
   * validation function to resolve. `false` otherwise.
   */
  validating: boolean;
...
> {
   ...
   * {@linkcode Config.onSubmit | onSubmit}.
   *
   * If `beforeSubmit` returns `false`, the submission will be aborted. If one
   * of your fields returns `false` on `beforeSubmit`, other fields may not have
   * their `beforeSubmit` called, as the submission is aborted on the first one
   * that returns `false`.
   */
   ...
   * {@linkcode FieldConfig.initialValue | initialValue} is `undefined` and the
   * value from the form's {@linkcode Config.initialValues | initialValues} is
   * also `undefined`. The field will be `dirty` when `defaultValue` is used.
   */
  defaultValue?: FieldValue;
  /**
   * A callback that will return a field-level validation function to validate a
   * single field value. The validation function should return an error if the
   * value is not valid, or `undefined` if the value is valid.
   */
  getValidator?: GetFieldValidator<FieldValue, FormValues>;
  /**
   * The initial value for the field. This value will be used to calculate
   * `dirty` and `pristine` by comparing it to the current value of the field.
   * If you want field to be `dirty` upon creation, you can set one value with
   * `initialValue` and set the value of the field with
   * {@linkcode FieldConfig.defaultValue | defaultValue}.
   *
   * The value given here will override any `initialValues` given to the entire
   * form.
   */
  ...
  isEqual?: IsEqual;
  /**
   * If `true`, no form or field listeners (apart from the one currently
   * registering) will be notified of anything that has changed with the
   * registration of this field. _Shhhh!!_
   *
   * This can be useful to do a "dry run" of registering a field and immediately
   * unregistering it to get an initial state for that field without disturbing
   ...
   *
   * - If `undefined`, _every_ field will be validated when this one changes;
   * - If `[]`, _only this field_ will have its field-level validation function
   *   called when it changes;
   * - If other field names are specified, those fields _and this one_ will be
   *   validated when this field changes.
   */
...
> extends Partial<
  ...
  /** Whether any fields have been modified since the last submit. */
  modifiedSinceLastSubmit: boolean;
  /** The form values at the last time the form was submitted. */
  lastSubmittedValues?: FormValues;
  /**
   * Whether the form was reset while in the process of being submitted (i.e.
   * while {@linkcode FormState.submitting | submitting} was `true`).
   */
  resetWhileSubmitting: boolean;
  /**
   * The total count of asynchronous validations that are currently being
   * awaited.
   */
  validating: number;
}
...
/** The following items exist on the object returned by `createForm()`. */
export interface FormApi<FormValues extends FormValuesShape = FormValuesShape> {
   ...
   * {@linkcode Config.destroyOnUnregister | destroyOnUnregister} config
   * setting.
   */
  ...
  /**
   * Returns the state of a specific field, of type {@linkcode FieldState}, as it
   * was last reported to its listeners, or `undefined` if the field has not
   * been registered.
   */
  ...
  /**
   * Initializes the form to the values provided. All the values will be set to
   * these values, and `dirty` and `pristine` will be calculated by performing a
   * shallow-equals between the current values and the values last initialized
   * with. The form will be `pristine` after this call.
   */
  initialize: (
    data:
  ...
  /** The state-bound versions of the mutators provided to {@linkcode Config}. */
   ...
   *
   * By default, `pauseValidation` also prevents all notifications being fired
   * to their subscribers. This is done performance reasons. However, if
   * notifications are still needed while validation is paused, you can pass
   * `false` to `pauseValidation`.
   */
  pauseValidation: () => void;
  /**
   * Registers a new field and subscribes to changes to it. **The `subscriber`
   * will _only_ be called, when the values specified in `subscription`
   * change.** More than one subscriber can subscribe to the same field.
   *
   * This is also where you may provide an optional field-level validation
   * function that should return `undefined` if the value is valid, or an error.
   * It can optionally return a `Promise` that _resolves_ (not rejects) to
   ...
   * Or empties all the values if the form was not initialized. If you provide
   * {@linkcode Config.initialValues | initialValues} they will be used as the
   * new initial values.
   *
   * Note that if you are calling `reset()` and not specify new initial values,
   * you must call it with no arguments. Be careful to avoid things like
   * `promise. catch(reset)` or `onChange={form.reset}` in React, as they will
   * get arguments passed to them and reinitialize your form.
   */
  reset: (initialValues?: InitialFormValues<FormValues>) => void;
  /**
   * Resets all of a field's flags (e.g. `touched`, `visited`, etc.) to their
   * initial state.
   */
  resetFieldState: (name: string) => void;
  /**
   * Resets all form and field state. Same as calling `reset(initialValues)` on
   * the form and {@linkcode FormApi.resetFieldState | resetFieldState()} for
   * each field. Form should be just as it was when it was first created.
   */
  restart: (initialValues?: InitialFormValues<FormValues>) => void;
  /**
   * Resumes validation paused by
   * {@linkcode FormApi.pauseValidation | pauseValidation()}. If validation was
   * blocked while it was paused, validation will be run.
   */
   ...
   *
   * Useful for controlling arbitrary top-level state related to your form. For
   * example:
   *
   * - You can use it to track "step stages" in a multi-step wizard form.
   * - You can use it to pass API responses back into your component in
   *   `handleSubmit`.
   */
   ...
   * `undefined` or a `Promise` depending on the nature of the
   * {@linkcode Config.onSubmit | onSubmit} configuration value given to the form
   * when it was created.
   */
  submit: () => Promise<SubmissionErrors> | undefined;
  /**
   * Subscribes to changes to the form. **The `subscriber` will _only_ be called
   * when values specified in `subscription` change.** A form can have many
   * subscribers.
   *
   * Related:
   *
...
> {
   ...
   *
   * Note that the fields are kept in a flat structure, so a "deep" field like
   * `"shipping.address.street"` will be at the key `"shipping.address.street"`,
   * with the dots included.
   */
  fields: {
  ...
  /**
   * The last form state sent to form subscribers. The object is very similar to
   * {@linkcode FormState}.
   */
 ...
 *
 * This is the bound version of the mutator that's available from the
 * {@linkcode FormApi}.
 */
 ...
 *
 * These are the bound versions of the mutators that are available from the
 * {@linkcode FormApi}.
 */
...
/**
 * Tools that will be passed into the functions used as
 * {@linkcode Config.mutators | mutators}.
 */
export interface Tools<FormValues extends FormValuesShape = FormValuesShape>
  ...
  /**
   * A utility function to rename a field, copying over its value and field
   * subscribers. _Advanced usage only_.
   *
   * Related:
   *
   * - {@linkcode MutableState}
   ...
   *
   * This can be useful for inserting a new field that has the same name as an
   * existing field.
   */
  resetFieldState: (name: string) => void;
  /**
   * A utility function to set any arbitrarily deep value inside an object using
   * dot-and-bracket syntax (e.g. `"some.deep.values[3].whatever"`). Note: it
   * does **not** mutate the object, but returns a new object.
   *
   * Related:
   *
   * - Field Names
  ...
  /**
   * A utility function to compare the keys of two objects. Returns `true` if
   * the objects have the same keys.
   */
 ...
 *
 * This is the unbound version of the mutator that gets passed into the form
 * config.
 */
export type Mutator<
  Arguments extends any[] = [],
 ...
 *
 * This are the unbound versions of the mutators that get passed into the form
 * config.
 */
...
export interface Config<FormValues extends FormValuesShape = FormValuesShape> {
  /**
   * A callback for debugging that receives the form state and the states of all
   * the fields. It's called _on every state change_.
   *
  ...
  /**
   * If `true`, the value of a field will be destroyed when that field is
   * unregistered. Defaults to `false`. Can be useful when creating dynamic
   * forms where only form values displayed need be submitted.
   */
  destroyOnUnregister?: boolean;
  /**
   * An arbitrary value for the initial status of the form. If the form is
   * reset, this value will be restored.
   */
   ...
   *
   * If you are using Typescript, these values must be the same type as the
   * object given to your {@linkcode Config.onSubmit | onSubmit} function.
   */
  initialValues?: InitialFormValues<FormValues>;
  /**
   * If `true`, only pristine values will be overwritten when
   * `initialize(newValues)` is called. This can be useful for allowing a user
   * to continue to edit a record while the record is being saved
   * asynchronously, and the form is reinitialized to the saved values when the
  ...
  mutators?:
    ...
    | Mutators<FormValues>;
  /**
   * Function to call when the form is submitted. There are three possible ways
   * to write an {@linkcode Config.onSubmit | onSubmit} function:
   *
   * ### 1. Synchronous
   *
   * Returns `undefined` on success, or an `Object` of submission errors on
   * failure.
   *
   * ### 2. Asynchronous with a callback
   *
   * Returns `undefined`, calls `callback()` with no arguments on success, or
   * with an `Object` of submission errors on failure.
   *
   * ### 3. Asynchronous with a `Promise`
   *
   * Returns a `Promise<SubmissionErrors>` that resolves with no value on
   * success or _resolves_ with an object of submission errors on failure. The
   * reason it _resolves_ with errors is to leave rejection for when there is a
   * server or communications error.
   *
   * ### Submission Errors
   *
   * Submission errors must be in the same shape as the values of the form. You
   * may return a generic error for the whole form (e.g. `'Login Failed'`) using
   * the special `FORM_ERROR` string key.
   *
   * Related:
   *
   * - {@linkcode FormApi}
  ...
  /**
   * A whole-record validation function that takes all the values of the form
   * and returns any validation errors. There are two possible ways to write a
   * `validate` function:
   *
   * ### 1. Synchronous
   *
   * Returns `{}` or `undefined` when the values are valid, or an object of
   * validation errors when the values are invalid.
   *
   * ### 2. Asynchronous with a `Promise`
   *
   * Returns a `Promise<ValidationErrors>` that resolves with no value on
   * success or _resolves_ with an object of validation errors on failure. The
   * reason it _resolves_ with errors is to leave _rejection_ for when there is
   * a server or communications error.
   *
   * ### Validation Errors
   *
   * Validation errors must be in the same shape as the values of the form. You
   * may return a generic error for the whole form using the special
  ...
  /**
   * If `true`, validation will happen on blur. If `false`, validation will
   * happen on change. Defaults to `false`.
   */
 ...
 * [decorates](https://en.wikipedia.org/wiki/Decorator_pattern) a form by
 * subscribing to it and making changes as the form state changes, and returns
 * an {@linkcode Unsubscribe} function to detach itself from the form. e.g.
